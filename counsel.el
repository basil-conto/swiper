...
(defun counsel--join-cmd (cmd &optional placeholder)
  ""
  (if (stringp cmd) cmd
    (mapconcat (let ((placeholder (if placeholder "%s" (make-symbol ""))))
                 (lambda (arg)
...
(defun counsel-M-x-action (cmd)
  ...
  (cond ((bound-and-true-p amx-initialized)
        ...
        ((bound-and-true-p smex-initialized-p)
         (smex-rank cmd)))
  (setq prefix-arg (or ivy-current-prefix-arg current-prefix-arg))
...
currently checked out."
  (cl-mapcan (lambda (line)
             ...
             (let ((default-directory (counsel-locate-git-root)))
               (counsel--sl "git --no-pager branch -vv --all --no-color"))))
