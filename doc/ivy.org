...
- =ivy-confirm-face= ::
     ...
     Some completion sessions allow exiting with arbitrary input text,
     but require confirmation when the input does not correspond to
     one of the given completion candidates.

     The confirmation prompt uses this face.
     ...
     means that commands that visit files (e.g., =find-file=) or
     buffers (e.g., =switch-to-buffer=) will ask before visiting
     a file or buffer name that does not exist.

     More concretely, if you call =find-file=, type in a nonexistent
     file name such as "eldorado", and press ~RET~ --- the prompt will
     have "(confirm)" appended.  Press ~RET~ once more to confirm, or
...
- =ivy-match-required-face= ::
     ...
     take arbitrary input, the "(match required)" prompt signals this
     constraint.
