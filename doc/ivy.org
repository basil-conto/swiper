...
is the selected candidate. The action is executed in the context of
the initial window from which the completion started.
...
(defun ivy-copy-action (x)
  (kill-new x))

(ivy-set-actions
 t
 '(("i" ivy-copy-to-buffer-action "insert")
   ("c" ivy-copy-action "copy")))
#+end_src

Then in any completion session, ~M-o c~ invokes =ivy-copy-action=, and
