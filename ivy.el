...
(defvar ivy-minibuffer-map
  (let ((map (make-sparse-keymap)))
    ...
    map)
  "Keymap used in the minibuffer during Ivy completion.")
...
(defun ivy-exit-with-action (action &optional exit-code)
  ...
  (let ((oaction (ivy-state-action ivy-last)))
    (ivy-set-action (lambda (x)
                      (funcall action x)
                      (ivy-set-action oaction))))
...
bound to `self-insert-command'."
  (declare (obsolete nil "0.14.3 (2024-06-09)"))
  (let* ((alist (assoc key
                       (plist-get
                        ivy--actions-list
                        caller)))
...
(defvar ivy-switch-buffer-map
  (let ((map (make-sparse-keymap)))
    ...
    map)
  "Keymap used during `ivy-switch-buffer' completion.")
...
(defvar ivy-reverse-i-search-map
  (let ((map (make-sparse-keymap)))
    ...
    map)
  "Keymap used during `ivy-reverse-i-search' completion.")
